...
## Follow-up: CC Switch official-route authentication correction
- [complete] User clarified Codex routing targets an OpenAI Official OAuth account. Inspect v3.20.3 providers/codex.rs and forwarder.rs before prescribing Pi API-key settings.
- [complete] Confirmed official Codex route uses inbound OAuth Authorization passthrough and managed account/session validation; a placeholder API key is insufficient. Correct prior conditional configuration advice for this concrete route.
- [complete] Recommend documented Claude-tab Codex OAuth Messages bridge for gateway-owned credentials, or a separate OAuth-aware client adapter for the current route. Preserve original openai-codex image-auth chain. No source/config/account changes or live requests.

## Current: Delegated subscription-to-API decision analysis
- [complete] Launched three fresh-context read-only lanes in workflow c9ba5d62-ee2f-45c2-abda-d2483a43db74: CC Switch upstream capabilities, minimal multi-model boundaries, and image-plugin credential compatibility.
- [complete] Consumed native completion and workflow receipt: all three lanes completed; installed 3.20.3 explicitly addressed in researcher report.
- [complete] Parent verified tagged CC Switch subscription guide/router/handlers/proxy service/auth preservation docs and current image credential/JWT/endpoint contracts. Generic Chat compatibility remains unproven; Images routes exist but plugin cannot use them unchanged.
- [complete] Synthesize recommendation: evaluate existing CC Switch before self-building; separate model capabilities; preserve original openai-codex auth and image direct path while adding a new chat provider. No code/config changes, credential inspection, or live integration tests.
- Validation: target plugin git status unchanged; planning git diff --check passed. Child reports are managed artifacts, not repository source. No test suite was run for this analysis-only task.
- User explicitly requested subagents. Use one async workflow, managed distinct output artifacts, no nested delegation. Preserve existing planning changes and unrelated deleted files.

## Current: Codex credentials as a reusable API (feasibility only)
- [complete] Inspect pi-codex-official architecture and relevant installed Pi documentation; no credentials read or implementation changed.
- [complete] Distinguish public API-key access from a subscription-backed local API adapter using official Codex auth docs and installed transport source.
- [complete] Recommend a separate loopback API bridge preserving the plugin, with explicit protocol/refresh/quota/security boundaries. No live model requests or delegation.
- Starting state: unrelated deleted interview files; preserve them and all existing planning history.
- Fetch note: OpenAI Help billing article returned HTTP 403; official Codex authentication page fetched successfully. Use its explicit subscription/API billing distinction rather than repeatedly fetching the blocked page.
