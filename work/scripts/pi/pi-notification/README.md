...
> **UX-1 通知设置**（单一入口 + 任务导向首页 + 渐进高级设置 + 直接测试 + Ctrl+S 单项固化）。
...
装完确认一下：TUI 里跑 `/notify`，首页聚焦总开关、提醒场景、接收方式、测试通知和更多设置；按 `Ctrl+O` 可查看状态与诊断里的 `pi-notification: 开启/关闭`。
...
`Ctrl+O` 的状态与诊断页显示时段是否当前生效；`Ctrl+T` 测试通知会绕过免打扰并提示，但仍受总开关和等级门槛约束。静默时段关闭时，界面把时间与例外收起为可展开的一行。
...
↑↓ 移动焦点 · Enter 打开/选择 · Space 快速切换字段或提醒场景 · Esc 返回上一级 · ? 字段详情
Enter 只改本对话 · Ctrl+S 保存单项为以后默认 · Ctrl+T 测试通知 · Ctrl+R 重读 · Ctrl+O 状态与诊断
```

首页只保留常用决策和验证入口；复杂配置放进「更多设置」：
...
- **提醒时机**里每个事件一行（开关、级别、接收位置摘要）；Space 直接开关该场景，Enter 打开该事件的详细规则与参数。
  **关闭的规则仍能进入配置**，页面说明“启用后生效”。
- **Space 快速切换**：布尔字段和提醒场景就地开关；集合/复杂字段打开候选列表；其它导航与动作行不响应 Space。
- **更多设置**包含提醒级别、提醒内容、免打扰时段、投递与频率；状态/诊断、重读配置和搜索也在此页。
...
- **搜索**：更多设置里的“搜索设置”或 `/` 打开，按中文名称、分组、内部配置 ID、渠道 ID 与输入提示筛选。
  结果携带路径，能定位到高级或已收起的字段（含当前不生效的字段）；打开只定位与预配置，
  **不会为了定位自动开启功能**，不生效的字段在列表与 `?` 详情里注明“尚未生效”及原因。
- **通知预览**在“更多设置 → 提醒内容”里：用固定示例数据生成正文示意，标注“示例（不会发送）”，
...
| 入口 | 作用 |
|---|---|
| `/notify`（TUI） | 打开轻量首页；Enter 浏览提醒时机/接收方式，Space 快速开关场景，Ctrl+S 固化当前字段为用户默认 |
...
| `/` | 打开搜索（与更多设置里的“搜索设置”相同）；输入筛选，Enter 打开结果，Esc 返回 |
| `a`（字段详情） | 沿用以后默认：只清除此项本对话覆盖 |
| `d`（字段详情） | 恢复此项内置默认：确认后同时清除用户默认与本对话覆盖（默认焦点为取消） |
| `Ctrl+S` | 把该字段已生效的值固化为用户级默认（单项稀疏写盘，渠道开关不带动其他渠道） |
| `Ctrl+T` | 发送一条测试通知，快速检查本机提醒。**绕过免打扰/合并/冷却**（仍受总开关/等级门槛/去重约束）；免打扰期间会说明本次测试例外 |
...
| `email` | QQ SMTP/TLS | 默认关闭；需填写发件与收件地址、QQ 邮箱授权码，并在需要提醒的场景里选择邮箱 |
...
`/notify → 接收方式 → 邮箱提醒` 开启邮箱并配置 QQ 发件地址、收件人（逗号分隔，支持其他邮箱服务商）和主题前缀。Enter/Space 只对本对话生效，Ctrl+S 可把各项保存为以后默认。邮箱默认关闭；启用后仍需到「提醒时机」逐类选择邮箱，默认场景不会自动增加邮件。要让正常 AI 回复同时触发本机和邮箱通知，请将「运行完成 → 渠道」设为「本机提醒」+「邮箱」，并分别 Ctrl+S 保存邮箱开关和场景接收方式。邮箱测试只验证邮件链路，不会替你启用正常事件路由。正常邮件的标题/正文复用本机通知内容（邮件标题额外带主题前缀），不会默认发送完整 AI 回复。

1. 在 QQ 邮箱网页设置中开启 SMTP 服务并生成**授权码**（不是 QQ 登录密码）；预置服务器为 `smtp.qq.com:465`、TLS。
2. 在邮箱页选择「设置邮箱授权码（遮蔽输入）」，在 Pi 的隐藏输入中输入刚生成的授权码；**界面安全保存仅支持 Windows**，授权码写入当前 Windows 用户的凭据管理器，不写进配置、会话或日志。其它系统暂不支持界面保存；旧的 `PI_NOTIFY_QQ_SMTP_AUTH_CODE` 仍可作为兼容回退，并建议从 Pi 启动器或秘密管理器注入，不要写入项目文件。
3. `/notify → 接收方式 → 邮箱提醒 → 发送邮箱测试` 只向配置的收件人发固定文本；测试前需启用邮箱并填好发件、收件地址和授权码。缺少前提时界面会给出下一步；“已提交”或 SMTP 接受均不保证最终进入收件箱，需检查收件箱/垃圾邮件以及更多设置里的状态统计。邮箱页的蓝色「QQ 邮箱网页版设置 ↗」链接通过 Pi 的默认浏览器打开官方登录页（`https://mail.qq.com/`）；登录后进入「设置 → 帐户」开启 SMTP 并生成授权码。
...
MSYS_NO_PATHCONV=1 node test/settings-ui.mjs        # 设置界面纯组件：首页/导航/搜索/预览/恢复/条件展示/全页渲染扫描
...
通知有没有显示在你的终端/通知中心，需要你自己确认——用界面里的 `Ctrl+T` 测试通知：
...
- `Ctrl+T` 的本机通知显示、`Ctrl+O` 状态与诊断排版、首页/更多设置导航、提醒场景 Space 开关、`Ctrl+S` 反馈与邮箱安全说明。
...
| 设置组件（N/SE/PR/RS/RA/R/K/D/C/I/V/F/S/X，54 项） | 任务导向首页与更多设置分层、友好渠道名/邮箱状态、邮箱子页和平台安全说明、直接测试通知、分类/规则/字段导航栈与 Esc 逐级返回、提醒场景 Space 开关、状态/输入/帮助/搜索返回原入口并保留父级焦点、条件收起（免打扰/规则关闭/immediate 专用窗口）、footer 按焦点给键、窄屏菜单标签优先、**全页面 20/24/30/40/80 列渲染扫描（无 `undefined`/`[object`/`null` 占位且不超宽）**、搜索覆盖全字段并标注未生效、纯本地预览零投递、`a`/`d` 单项恢复与确认页默认取消、恢复失败不改盘且不提前清会话值、字段行内联当前值（中文）、标记列固定占位、Ctrl+S 标记迁移与状态行确认、稀疏单项落盘、Ctrl+S 只写已生效值、集合多选与整数组保存、数值/时间自定义输入校验、强制静默作额外限制、overlay 优先级与 base 继承、fork 不继承 |
...
extensions/index.ts   薄接线：注册 + 形状转换 + 配置装配；注册 /notify 与 --no-notify
   ...
   │    └─ ui.ts       设置界面：首页/更多设置/规则导航 / 友好状态 / 快捷操作 / Ctrl+S / 状态与诊断
