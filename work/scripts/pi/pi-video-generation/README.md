...
每次保存一个 H.264 MP4。生成完成后，会话中会显示 **Open original video** 原视频链接，与生图插件的原图链接展示方式一致，无需再去文件管理器查找。

- 支持超链接的终端（如 Windows Terminal）可按 **Ctrl+单击** 链接，由终端和系统默认应用打开 MP4；不保证一定使用浏览器。
- 不支持超链接的终端仍显示完整 `file:///...` URL，可复制后手动打开。
- 链接保留在会话记录中，重新加载后仍可展示；已有的视频记录也会根据保存路径显示链接，无需重新生成。文件被移动或删除后，原链接将失效。
- `/video resume <task-id>` 恢复任务并下载成功后，同样显示原视频链接。RPC 成功通知包含不带终端转义符的 URL。
- 不生成额外 HTML、不自动启动播放器，也不把 MP4 读入终端渲染。TUI 成功通知保留保存路径，不重复展示链接。

模型生成的音轨随 MP4 保存；扩展不提供独立音频开关、首尾帧或多段视频流程，也不在终端内播放视频。
...
```bash
npm test
# Pi 仅全局安装、无法自动解析 SDK 时，传入实际 SDK 文件 URL：
npm test -- file:///D:/Nodejs/node_modules/@earendil-works/pi-coding-agent/dist/index.js
```

测试使用宿主的 Jiti/TUI 和临时合成文件，覆盖链接发布、历史记录、非法路径、OSC 8 点击目标与换行、主题更新及 RPC/不支持终端的降级；不读取真实凭据、不请求生视频服务。不等同于完整生成/恢复流程或实际终端点击验收。
