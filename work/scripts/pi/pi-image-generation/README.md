...
生成成功后保留原有保存路径和终端图片预览，直接显示原图的 `file:///...` 链接，**不再生成 HTML 预览页**。TUI 每张图片的会话记录各显示一个原图链接；保存通知保留 `Saved` 和图片路径，不重复显示链接。链接使用主题链接色（`mdLink`）与下划线；检测到终端支持时，输出真正的 **OSC 8 超链接**。RPC / 非 TUI 的通知逐张提供无控制码的完整 URL。

**不会自动打开图片**。Windows Terminal 中通常可 **Ctrl+点击** 链接，由终端和系统文件关联决定打开方式，可能是图片查看器，也可能是浏览器，**不保证在浏览器中打开**。其他终端的点击方式以其设置为准。CMD / PowerShell 是 shell，能否点击取决于承载它们的终端窗口：传统 Windows 控制台不支持 OSC 8，即使文字有颜色也无法 Ctrl+点击。此时请在支持链接的终端运行 Pi，或复制保存路径用图片查看器打开，也可复制完整 URL 到浏览器地址栏。
...
链接从会话中保存的图片路径生成，重新加载后仍可查看；旧会话也直接显示原图链接，不再依赖历史图集。已有图片和 HTML 文件不会删除或改写。图片移动或删除后，原路径链接将失效。

不启动服务器、不上传图片，不为链接额外生成文件。目前仍是每次请求一张，未新增批量生成参数。
...
宿主检查按实际配置覆盖已启用模型生成/编辑的 mock 请求，并使用合成禁用模型验证运行时不注册、在认证和 fetch 前拒绝；还覆盖命令/设置、扩展重新初始化，以及逐张原图链接发布、历史会话渲染、OSC 8 目标与换行、主题更新、单张/多张结果的 TUI 链接去重、RPC/不支持终端的降级、终端预览失败时保留链接，以及不额外生成 HTML 文件。它**不等同于实际 TUI 操作或完整 AgentSession.reload 集成验证**。实际终端秘密输入、账户设置交互和真实生图应另行验收；真实请求会消耗额度，自动测试不会发送。
