...
try {
 ...
 // Publish a batch without opening an app, invoking an agent turn or writing HTML.
 const outputDir=path.join(dir,'.pi','generated-images');
 const legacyGalleryPath=path.join(outputDir,'gallery-historical.html');
 const legacyHtml='<!doctype html><title>Historical gallery</title>';
 await writeFile(legacyGalleryPath,legacyHtml);
 const outputFilesBefore=(await readdir(outputDir)).sort();
 ...
 assert.deepEqual(entries.map(entry=>entry.data),generatedEntries);
 assert.ok(entries.every(entry=>!('galleryPath' in entry.data)));
 const imageUrl=pathToFileURL(entries[0].data.path).href;
 for(const entry of generatedEntries){
  const url=pathToFileURL(entry.path).href;
  assert.equal(notices.at(-1).split(url).length-1,1);
 }
 assert.ok(notices.at(-1).includes(`Saved ${expectedImageCount} image`));
 assert.doesNotMatch(notices.at(-1),/\x1b|Browser preview|\.html/); // RPC stays plain text.
 assert.deepEqual((await readdir(outputDir)).sort(),outputFilesBefore);
 // Reloaded renderer derives links from persisted paths; historical gallery metadata is ignored.
 await extension(pi);
 ...
 for(const data of [entries[0].data,{...entries[0].data,galleryPath:legacyGalleryPath}]){
  const rendered=renderer({data:JSON.parse(JSON.stringify(data))},{},theme).render(1000).join('\n');
  assert.ok(rendered.includes(imageUrl));
  assert.doesNotMatch(rendered,/Browser preview|\.html/);
 }
 // Publication no longer needs a writable cwd and cannot fail while creating a preview file.
 const noticeCount=notices.length;
 await publishOutcome(pi,{...ctx,cwd:generatedEntries[0].path},{entries:[generatedEntries[0]],texts:[]});
 assert.equal(entries.length,expectedImageCount+1);
 assert.equal(notices.length,noticeCount+1);
 assert.match(notices.at(-1),/Saved 1 image/);
 assert.ok(notices.at(-1).includes(imageUrl));
 assert.deepEqual(await readFile(generatedEntries[0].path),bytes);
 // Missing/corrupt terminal previews retain the direct link without crashing the renderer.
 const corruptPath=path.join(outputDir,'corrupt.png');
 await writeFile(corruptPath,'not an image');
 for(const filePath of [path.join(outputDir,'missing.png'),corruptPath]){
  const rendered=renderer({data:{...generatedEntries[0],path:filePath}},{},theme).render(1000).join('\n');
  assert.match(rendered,/Image preview unavailable/);
  assert.ok(rendered.includes(pathToFileURL(filePath).href));
 }
 await rm(corruptPath);
 for(const filePath of [undefined,'.pi/generated-images/relative.png',path.join(dir,'outside.png')]){
  const rendered=renderer({data:{...generatedEntries[0],path:filePath}},{},theme).render(1000).join('\n');
  assert.match(rendered,/invalid path/);
  assert.ok(!rendered.includes('file:///'));
 }
 // Real OSC 8 targets and styling, not merely a string containing a file URL.
 const {formatImageLink}=await jiti.import('../src/preview-link.ts');
 ...
 try {
  ...
  const specialPath=path.join(dir,'.pi','generated-images','原图 space # % "\x1b.png');
  const specialUrl=pathToFileURL(specialPath).href;
  assert.equal(fileURLToPath(specialUrl),specialPath);
  assert.match(specialUrl,/%20/);assert.match(specialUrl,/%23/);assert.match(specialUrl,/%25/);
  assert.match(specialUrl,/%1B/i);
  const formatted=formatImageLink(specialPath,theme);
  ...
  assert.equal(formatImageLink(specialPath),specialUrl);
  ...
  assert.equal(getOsc8LinkAtColumn(linkLine,0),imageUrl);
  ...
  // Single and batch results display one original link PER image across the TUI
  ...
  for(const hyperlinks of [true,false]){
   ...
   for(const batch of [[generatedEntries[0]],generatedEntries]){
    ...
    assert.ok(published.every(entry=>!('galleryPath' in entry.data)));
    ...
    assert.equal(visible.split('Open original image').length-1,batch.length);
    assert.doesNotMatch(visible,/Browser preview|\.html/);
    for(const entry of batch){
     const tuiUrl=pathToFileURL(entry.path).href;
  ...
  const fallback=formatImageLink(specialPath,theme);
  ...
  assert.equal(stripTerminalSequences(fallbackLine).trimEnd(),imageUrl);
 } finally {setCapabilities(originalCapabilities);}
 assert.deepEqual((await readdir(outputDir)).sort(),outputFilesBefore);
 assert.equal(await readFile(legacyGalleryPath,'utf8'),legacyHtml);
 for(const entry of generatedEntries)assert.deepEqual(await readFile(entry.path),bytes);
 console.log('Host-backed offline checks passed: mocked generation/edit, safe transport diagnostics/logs through SDK and RPC, direct original links without HTML writes, historical entries/reload, preview failure, styled OSC 8 targets, wrapping, theme refresh, per-image TUI link deduplication and RPC/unsupported-terminal fallback.');
